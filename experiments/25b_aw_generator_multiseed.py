"""Experiment 25b — AW-generator validation, MULTI-SEED (statistical honesty).

E25 single-seed gave noisy, partly contradictory results (V1 clean=0.622 vs E6b's
0.753; V2 old-sim=0.810 unexpectedly high). Single-seed on an n=700 CinC test split
is too noisy to trust. E25b runs the SAME comparison across 5 seeds and reports
mean±std so the acceptance test (does V3 generated beat V1 clean?) is statistically
honest, not a coin flip.

Same setup as E25: binary NORM-vs-AF, test on real CinC (validated AW proxy).
Per seed: reshuffle CinC ref/test split AND re-init model. Variants:
  V1 clean Lead-I · V2 old heavy sim · V3 generated AW · V4 clean+generated · V5 oracle

Reuses E25's loaders and the AWGenerator. The verdict is the sign and significance
of (V3 - V1) and (V4 - V1) across seeds.
"""
...
RESULTS = ROOT / "results" / "25b_aw_generator_multiseed"
...
def run_seed(seed, tr, tr_leadI, tr_y, cinc):
    ...
    # transfer function fit on this seed's ref split (labels unused)
    ref_sigs = [r["sig"] for r in cinc_ref]
    H = build_transfer_function(tr_leadI, ref_sigs, fs=FS, siglen=SIGLEN)
    gen = AWGenerator(H, fs=FS, siglen=SIGLEN, noise_level=0.03, baseline_boost=0.04, seed=seed)
    ...
    run("V3_generated", gen_sigs, tr_y)
    run("V4_clean_gen", tr_leadI + gen_sigs, tr_y + tr_y)
    # oracle
...
def main():
    ...
    # aggregate
    keys = ["V1_clean", "V2_oldsim", "V3_generated", "V4_clean_gen", "V5_oracle"]
    agg = {}
    for k in keys:
    ...
    for k in ["V3_generated", "V4_clean_gen", "V2_oldsim"]:
        vk = np.array([per_seed[s][k] for s in seeds])
        d = vk - v1
    ...
    metrics = {
        ...
        "acceptance_test": "V3_generated and V4_clean_gen delta_vs_V1 > 0 across seeds",
        "honesty": ["5 seeds, n=700 CinC (350 test/seed)", "AF/NORM only",
                    "transfer fn fit toward CinC (real-AW proxy)", "generator preserves morphology (label-valid)"],
    }
...
def _plot(agg, keys):
    ...
    ax.set_title("E25b: AW-generator validation (multi-seed) — does generated beat clean?")
    ax.legend(fontsize=8, loc="upper left")
    for b, m, s in zip(bars, means, stds):
        ax.text(b.get_x()+b.get_width()/2, m+s+0.01, f"{m:.3f}", ha="center", fontsize=9)
    plt.tight_layout(); plt.savefig(RESULTS / "multiseed_validation.png", dpi=130); plt.close()
