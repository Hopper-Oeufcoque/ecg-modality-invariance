...
RESULTS = ROOT / "results" / "60_real_aw_sex"; RESULTS.mkdir(parents=True, exist_ok=True)
FS = 100.0; SIGLEN = 1000; SEEDS = 5; N_FOLDS = 5
...
def main():
    ...
    ax.set_title(f"E60 FIRST MEASURED real-AW outcome: SEX transfer (clinical->AW, n=243, {SEEDS} seeds)\nclean {A['clean'].mean():.3f} · calib {A['closed_aug'].mean():.3f} · scramble {A['band_scramble'].mean():.3f} · aligned {A['aligned'].mean():.3f} · oracle {A['oracle'].mean():.3f}")
    ax.legend(fontsize=8)
    fig.tight_layout(); fig.savefig(RESULTS/"real_aw_sex.png",dpi=110)
