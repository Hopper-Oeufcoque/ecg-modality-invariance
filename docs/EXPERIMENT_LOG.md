...
| E60 | **⭐ FIRST MEASURED real-Apple-Watch outcome: SEX transfer (PTB-XL→SJLIFE AW, 5 seeds, n=243 patients)** | clean 0.680; calibration 0.697 (+0.017 n.s.); scramble 0.679 (null); **aligned 0.712 (+0.031 vs clean 5/5, p=0.085)**; oracle 0.692; clinical_self 0.684 | ✅(prov) | **THE NORTH STAR, MEASURED ON REAL WATCH DATA (not CinC proxy, not a prediction). Enabled by Hop spotting gender_x in the SJLIFE CSV. THREE findings: (1) SEX MORPHOLOGY SURVIVES THE MODALITY SHIFT — clinical_self 0.684 ≈ clean-transfer 0.680; in-band QRS/T sex signal crosses clinical Lead-I→real AW with ~no loss (unlike fragile out-of-band rhythm, E38–47). (2) ALIGNMENT IS BEST ON REAL AW, UNANIMOUS — aligned 0.712, +0.031 vs clean on 5/5 seeds; the E51–56 CinC direction now holds on genuine Apple Watch. p=0.085 at 5 seeds (5/5 sign-test p=0.031) → PROVISIONAL, higher-seed confirm launched (E60b, 15 seeds). (3) THE SURPRISE: aligned (0.712) BEATS oracle (0.692) — leveraging 21.8k clinical + unlabeled pairs > training on scarce real-AW sex labels (n=243×~3 windows). Validates the whole premise: abundant clinical data + modality alignment > direct watch labels. CAVEATS (load-bearing): n=243/5-seed not yet <0.05; POPULATION shift (PTB-XL old/cardiac vs SJLIFE young/cancer-survivor) rides atop modality shift → helping despite double-shift is conservative; ~3 windows/patient; sex ≠ disease. Leakage-safe (patient-disjoint folds). calibration/scramble weaker here (sex is in-band, out-of-band levers idle — consistent w/ E44 dose-response).** |
